...
async function checkSpelling(text: string): Promise<{ resultOutput: string, corrections: Correction[] }> {
...
function parseSpellingResults(html: string): { resultOutput: string, corrections: Correction[] } {
    const parser = new DOMParser();
    const doc = parser.parseFromString(html, 'text/html');
    const scriptTag = Array.from(doc.scripts).find(script => script.text.includes('data = '));
    if (scriptTag) {
...
function formatSpellingResults(resultData: any[]): { resultOutput: string, corrections: Correction[] } {
    let resultOutput = "";
    const corrections: Correction[] = [];
    resultData.forEach((sentenceData, index) => {
        let highlightedText = sentenceData.str;
        sentenceData.errInfo.forEach((err: any, errIndex: number) => {
...
function replaceFirstOccurrenceWithPlaceholder(text: string, search: string, placeholder: string): string {
...
function createCorrectionPopup(corrections: Correction[], selectedText: string, start: any, end: any, editor: any) {
    ...
    <div id="correctionPopup" class="${themeClass}">
        ...
        <div class="preview-container">
            <div class="error-text">${highlightOriginalTextWithPlaceholders(selectedText, corrections, 'error')}</div>
            <div class="arrow">►</div>
            <div id="resultPreview" class="result-preview">${highlightOriginalTextWithPlaceholders(selectedText, corrections, 'preview')}</div>
        </div>
        <div class="content">
            <div id="correctionUI" class="correction-list">
                ...
                <div class="correction-item">
                    ...
                    <b>수정:</b>
                    ${correction.corrected.map((option, optIndex) => `
    ...
    function highlightOriginalTextWithPlaceholders(text: string, corrections: Correction[], type: string): string {
        ...
        corrections.forEach((correction, index) => {
            text = text.replace(`{placeholder_${index}}`, `<span id="${type}_correction${index}" style="color: var(--color-red); font-weight: bold;">${correction.original}</span>`);
        });
        return text;
    }
    ...
    function highlightCorrectedText(corrections: Correction[]): void {
        corrections.forEach((correction, index) => {
            const selectedOptionElement = document.querySelector(`input[name="correction${index}"]:checked`) as HTMLInputElement;
            let selectedOption = selectedOptionElement ? selectedOptionElement.value : correction.original;
            const customTextElement = document.getElementById(`customCorrection${index}`) as HTMLInputElement;
    ...
    document.getElementById("applyCorrectionsButton")?.addEventListener("click", () => {
        ...
        editor.replaceRange(document.getElementById("resultPreview")?.innerHTML.replace(/<\/?span[^>]*>/g, "") || "", start);
...
export default class SpellingPlugin extends Plugin {
    async onload() {
        console.log('Loading SpellingPlugin');
        this.addCommand({
            id: 'check-spelling',
            name: 'Check Spelling',
            editorCallback: async (editor) => {
                ...
                if (resultOutput === "" && corrections.length === 0) {
                    ...
                    createCorrectionPopup(corrections, selectedText, cursorStart, cursorEnd, editor);
                }
            }
        });
    }
    ...
    onunload() {
        console.log('Unloading SpellingPlugin');
    }
}
